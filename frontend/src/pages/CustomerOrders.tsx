...
export default function CustomerOrders() {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      setIsUpdating(true);
      await api.patch(`/customer-orders/${selectedRecord.id}/status`, {
        status: newStatus,
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="bg-surface shadow overflow-hidden sm:rounded-lg">
          <table className="min-w-full divide-y divide-border">
            <thead className="bg-background">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">Order #</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider">Customer Name</th>
              ...
              {isLoading ? (
                ...
                orders.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{row.customer_name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{row.location.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{new Date(row.created_at).toLocaleDateString()}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">{getStatusBadge(row.status)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      {(user?.role === 'ADMIN' || user?.role === 'SALES' || user?.role === 'OPERATIONS') && (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Update Customer Order Status">
        {selectedRecord && (
          <div className="space-y-4">
            <div>
              <p className="text-sm font-medium text-text-primary">Order: {selectedRecord.order_number}</p>
              <p className="text-sm text-text-secondary">Customer: {selectedRecord.customer_name}</p>
